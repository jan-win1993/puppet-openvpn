...
define openvpn::client (
  ...
  exec { "Create PEM file without text with OpenSSL for client ${name}":
    cwd         => "${server_directory}/${server}/download-configs/",
    command     => "/usr/bin/openssl x509 -in ${server_directory}/${server}/download-configs/${name}/keys/${name}/${name}.crt > ${server_directory}/${server}/download-configs/${name}/keys/${name}/${name}.pem",
